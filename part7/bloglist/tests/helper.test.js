...
describe("Returns author with most blogs", () => {
  ...
  test("when list has one entry", () => {
    ...
    assert.deepEqual(result, {
      ...
      blogs: listHelper.listWithOneBlog.length,
    });
  });
...
describe("Verify blog list application returns correct JSON from requests", () => {
  ...
  test("Verify making an HTTP request creates new blog post", async () => {
    ...
    const response = await api
      .post("/api/login")
      .send({
        username: listHelper.singleUser[0].username,
        password: listHelper.singleUser[0].password,
      });
  ...
  test("If likes is missing, default to 0", async () => {
    ...
    const testBlog = blogsAtEnd.filter(
      (b) => b.title === "Missing Likes Request",
    );
  ...
  test("PUT request to update an entry", async () => {
    ...
    await api.put(`/api/blogs/${id}`).send(testBlog).expect(201);
